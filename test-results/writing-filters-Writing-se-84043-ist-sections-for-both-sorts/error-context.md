...
- Name: writing-filters.spec.js >> Writing section filters >> should show list sections for both sorts
- Location: writing-filters.spec.js:60:3

# Error details

```
Error: locator.click: Error: strict mode violation: locator('[data-sort="views"]') resolved to 3 elements:
    1) <button data-sort="views" aria-pressed="false" data-astro-cid-ptvumqbc="" class="filter-btn filter-btn-views">Most Viewed</button> aka getByRole('button', { name: 'Most Viewed' })
    2) <div data-sort="views" data-featured="true" class="featured-essays" data-astro-cid-ptvumqbc="">…</div> aka locator('div').filter({ hasText: 'The Seam Buys a' }).nth(2)
    3) <ul data-sort="views" class="essay-list" data-featured="false" data-astro-cid-ptvumqbc="">…</ul> aka locator('ul').filter({ hasText: 'aesop 0.4.0 — Two Swappable' }).nth(1)

Call log:
  - waiting for locator('[data-sort="views"]')
